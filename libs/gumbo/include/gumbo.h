 ...
 * library.
 */
__declspec(dllexport) const char* gumbo_normalized_tagname(GumboTag tag);
 ...
 * no-op.
 */
__declspec(dllexport) void gumbo_tag_from_original_text(GumboStringPiece* text);
 ...
 * the library.
 */
__declspec(dllexport) const char* gumbo_normalize_svg_tagname(const GumboStringPiece* tagname);

/**
 * Converts a tag name string (which may be in upper or mixed case) to a tag
 * enum. The `tag` version expects `tagname` to be NULL-terminated
 */
__declspec(dllexport) GumboTag gumbo_tag_enum(const char* tagname);
__declspec(dllexport) GumboTag gumbo_tagn_enum(const char* tagname, unsigned int length);
 ...
 */
__declspec(dllexport) GumboAttribute* gumbo_get_attribute(const GumboVector* attrs, const char* name);
...
__declspec(dllimport) extern const GumboOptions kGumboDefaultOptions;
 ...
 */
__declspec(dllexport) GumboOutput* gumbo_parse(const char* buffer);

/**
 * Extended version of gumbo_parse that takes an explicit options structure,
 * buffer, and length.
 */
__declspec(dllexport) GumboOutput* gumbo_parse_with_options(
    const GumboOptions* options, const char* buffer, size_t buffer_length);

/** Release the memory used for the parse tree & parse errors. */
__declspec(dllexport) void gumbo_destroy_output(const GumboOptions* options, GumboOutput* output);
